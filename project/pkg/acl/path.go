...
func GetPathsForSetTraverse(p string, roles RoleMap, chanF *chan ufp.FilePathMode) {
	// boolean map for traverse role users
	mUsersT := make(map[string]bool)
...
func GetPathsForDelTraverse(p string, roles RoleMap, chanF *chan ufp.FilePathMode) {
	// boolean map for traverse role users
	mUsersT := make(map[string]bool)
	for _, u := range roles[Traverse] {
		mUsersT[u] = true
